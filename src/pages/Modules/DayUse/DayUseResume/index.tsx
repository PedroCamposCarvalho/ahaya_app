...
const DayUsePayment: React.FC = () => {
  ...
  const availabilityZeroToastOpts = useMemo(
    () => ({
      data: 'As vagas acabaram... lamentamos',
      textColor: '#fff',
      backgroundColor: '#c53030',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  const materialBiggerThanTicketsToastOpts = useMemo(
    () => ({
      data: 'Somente 1 raquete para cada ticket',
      textColor: '#fff',
      backgroundColor: '#c53030',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  useEffect(() => {
    socket.on('NewDayUseList', () => {
      api.get(`/dayUse/findAvailability?id_dayuse=${id}`).then(response2 => {
        ...
        if (availability === 0) {
          WToast.show(availabilityZeroToastOpts);
          navigation.goBack();
        }
      });
    });
  }, [availabilityZeroToastOpts, socket, id, limit, navigation]);
  ...
  const handleAddMaterial = useCallback(() => {
    if (materials + 1 > tickets) {
      WToast.show(materialBiggerThanTicketsToastOpts);
    } else {
      setMaterials(materials + 1);
    }
  }, [materials, tickets, materialBiggerThanTicketsToastOpts]);
