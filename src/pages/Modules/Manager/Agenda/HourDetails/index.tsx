...
const HourDetails: React.FC<PageProps> = ({ closeModal, data }) => {
  ...
  const singleAppointmentDeletedoastOpts = useMemo(() => {
    return {
      data: 'Reserva excluída com sucesso!',
      textColor: '#fff',
  ...
  const multipleAppointmentsDeletedoastOpts = useMemo(() => {
    return {
      data: 'Reservas excluídas com sucesso!',
      textColor: '#fff',
      backgroundColor: '#4bb543',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  const handleDeleteOnlyOneAppointment = useCallback(() => {
    ...
    api
      ...
      .then(response => {
        if (String(response.data) === 'true') {
          WToast.show(singleAppointmentDeletedoastOpts);
          closeModal();
        }
      })
      .catch(error => {
    ...
    data,
    singleAppointmentDeletedoastOpts,
    closeModal,
  ...
  const handleDeleteAllAppointments = useCallback(() => {
    ...
    api
      ...
      .then(response => {
        if (String(response.data) === 'true') {
          WToast.show(multipleAppointmentsDeletedoastOpts);
          closeModal();
        }
      })
      .catch(error => {
    ...
    data,
    closeModal,
    multipleAppointmentsDeletedoastOpts,
    generalErrorToastOpts,
  ]);
