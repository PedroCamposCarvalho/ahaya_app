...
const EditItem: React.FC = () => {
  const fillAllInputsErrorToastOpts = useMemo(
    () => ({
      data: 'Preencha todos os campos para salvar!',
      textColor: '#fff',
  ...
  const productUpdatedToastOpts = useMemo(
    () => ({
  ...
  const productDeletedToastOpts = useMemo(
    () => ({
  ...
  const permissionDeniedToastOpts = useMemo(
    () => ({
      data: 'Você deve dar permissão para atualizar sua foto de perfil',
      textColor: '#fff',
      backgroundColor: '#c53030',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  const handleSubmit = useCallback(() => {
    if (
      ...
      WToast.show(fillAllInputsErrorToastOpts);
    } else {
      setLoading(true);
      api.put('/store/updateProduct', product).then(response => {
        WToast.show(productUpdatedToastOpts);
        getProducts();
        navigation.goBack();
      });
    }
    ...
    product,
    fillAllInputsErrorToastOpts,
    productUpdatedToastOpts,
    getProducts,
    navigation,
  ]);
