...
const NonRetrieved: React.FC = () => {
  const productRetrievedToastOpts = useMemo(
    () => ({
      data: 'Produto retirado com sucesso!',
      textColor: '#fff',
  ...
  useEffect(() => {
    api.get('/store/findNonRetrievedPurchases').then(response => {
  ...
  const handleRetrieveProduct = useCallback(() => {
    api
      ...
      .then(response => {
        WToast.show(productRetrievedToastOpts);
        setModalOpened(false);
        handleRefresh();
      });
  }, [purchaseProps, productRetrievedToastOpts, handleRefresh]);
    ...
    <LinearGradient colors={['#006edb', '#273a9a']} style={{ flex: 1 }}>
      <Container>
        ...
        <Modal isVisible={modalOpened}>
          <ModalContainer>
            <ModalHeader>
              <ModalHeaderTitle>Opções</ModalHeaderTitle>
              <ModalHeaderButton onPress={() => closeModal()}>
                <MaterialIcon name="close" color="#fff" size={RFValue(20)} />
              </ModalHeaderButton>
            </ModalHeader>
            <ModalUpdateButton onPress={() => handleRetrieveProduct()}>
              <ModalUpdateButtonText>Retirar produto</ModalUpdateButtonText>
            </ModalUpdateButton>
          </ModalContainer>
        </Modal>
