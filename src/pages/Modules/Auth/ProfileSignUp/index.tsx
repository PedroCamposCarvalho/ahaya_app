...
const ProfileSignUp: React.FC = () => {
  const generalErrorToastOpts = useMemo(
    () => ({
      data: 'Verifique sua conexão e tente novamente!',
      textColor: '#fff',
  ...
  const invalidSsnToastOpts = useMemo(
    () => ({
      data: 'CPF Inválido',
      textColor: '#fff',
      backgroundColor: '#c53030',
  ...
  const ssnInUseToastOpts = useMemo(
    () => ({
  ...
  const emailInUseToastOpts = useMemo(
    () => ({
  ...
  const fillAllBlanksToastOpts = useMemo(
    () => ({
      data: 'Preencha todos os campos para continuar!',
      textColor: '#fff',
      backgroundColor: '#c53030',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  const incorrectPasswordCombinationToastOpts = useMemo(
    () => ({
  ...
  const agreeWithTermsToastOpts = useMemo(
    () => ({
      data: 'Você deve concordar com os TERMOS DE USO e a POLÍTICA DE PRIVACIDADE!',
      textColor: '#fff',
      backgroundColor: '#c53030',
      duration: WToast.duration.LONG,
      position: WToast.position.TOP,
  ...
  const zipCodeNotFoundToastOpts = useMemo(
    () => ({
  ...
  const handleSignUp = useCallback(
    async (data: SignUpFormData) => {
      try {
        ...
        if (gender === '') {
          WToast.show(fillAllBlanksToastOpts);
          throw Error();
        }
        if (!readTerms) {
          WToast.show(agreeWithTermsToastOpts);
          throw Error();
        }
        ...
        if (validateSSN(data.ssn)) {
          ...
          if (existingSsn === undefined) {
            ...
            if (existingEmail === undefined) {
              ...
              WToast.show(emailInUseToastOpts);
          ...
          WToast.show(invalidSsnToastOpts);
          setLoading(false);
        }
      } catch (error) {
        if (error instanceof Yup.ValidationError) {
          ...
          if (error.message === 'As senhas não coincidem') {
            WToast.show(incorrectPasswordCombinationToastOpts);
          } else {
            WToast.show(fillAllBlanksToastOpts);
          }
    ...
    [
      ...
      fillAllBlanksToastOpts,
      agreeWithTermsToastOpts,
      gender,
      readTerms,
      incorrectPasswordCombinationToastOpts,
      signIn,
    ],
  );
  ...
  const handleSearchZipCode = useCallback(() => {
    ...
    if (zipCode > '') {
      api
        ...
        .catch(error => {
          WToast.show(zipCodeNotFoundToastOpts);
          setZipCodeLoading(false);
        });
    }
  }, [zipCodeNotFoundToastOpts]);
  return (
    <KeyboardAvoidingView
      style={{ flex: 1 }}
