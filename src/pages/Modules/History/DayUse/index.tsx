...
const DayUse: React.FC = () => {
  ...
  const cancelSuccessToastOpts = useMemo(
    () => ({
  ...
  const cancelDayUse = useCallback(
    async (
      ...
      if (String(response) === 'true') {
        api
          ...
          .then(response2 => {
            WToast.show(cancelSuccessToastOpts);
            setLoading(false);
            setDayUses([]);
          })
          .catch(error => {
    ...
    [cancelSuccessToastOpts, generalErrorToastOpts, user.id],
  );
  ...
  }, [socket, user.id, generalErrorToastOpts]);
